...
<body>
    ...
    <script>
        ...
        async function updateSegments() {
            ...
            segmentsEl.innerHTML = data.segments.map(function(s) {
                var color = s.status === 'final' ? '#00ff00' : '#ffff00';
                return '<div>[' + s.start_time.toFixed(1) + 's - ' + s.end_time.toFixed(1) + 's] <span style="color:' + color + '">' + s.status + '</span>: ' + s.text + '</div>';
            }).reverse().join('');
        }
